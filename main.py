...
for i in range(5):
    enemy = Enemy(ENEMY_IMG, randint(0, ANCHO - 50), -
                  40, 50, 40, randint(1, 2))
    enemies.add(enemy)

for i in range(4):
    enemy1 = Enemy1(ENEMY_IMG, randint(0, ANCHO - 50), -
...
while run:
    ...
    points_text = font_1.render(f'PUNTOS: {puntos}', 1, GREEN)
    misses_text = font_1.render(f'FALLOS: {fallos}', 1, GREEN)
